...
// PROTOTYPE — throwaway. Floating variant switcher for the 3 combined
// multi-Program editor layouts, per the /prototype skill's UI convention.
// wayframe#125. Hidden in production by the host page's own dev-only gate.
import { useRouter, usePathname, useSearchParams } from "next/navigation";
import { useCallback, useEffect } from "react";

export const VARIANTS = [
  { key: "a", label: "A — Canvas over docked structure tabs" },
  { key: "b", label: "B — Left Program rail, canvas right" },
  { key: "c", label: "C — Band chip strip + structure drawer" },
] as const;

export type VariantKey = (typeof VARIANTS)[number]["key"];

export function useEditorVariant(): VariantKey {
...
export function EditorSwitcher() {
  const current = useEditorVariant();
