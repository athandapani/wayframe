...
//
// PROTOTYPE (wayframe#104, throwaway): ?merged=1 renders the All-Programs
// merged-view fixture, always in "header" (wayframe#100's resolved variant)
// — there's no new groupBandVariant to add, since a Program band is just a
// depth-0 SwimlaneGroup band (see prototype-merged-programs-fixture.ts).
"use client";
...
import { prototypeMergedProgramsFixture } from "@/components/timeline/__fixtures__/prototype-merged-programs-fixture";
...
function RoadmapTimelineDevPreviewInner() {
  ...
  const mergedMode = searchParams.get("merged") === "1";
  ...
  const baseFixture = mergedMode ? prototypeMergedProgramsFixture : groupsMode ? prototypeGroupsFixture : null;
  ...
  // way onMilestoneDateChange writes a moved date back. Shared by both
  // ?groups= and ?merged= since both key off the same
  // data.swimlaneGroups[].collapsed shape.
  const [collapsedIds, setCollapsedIds] = useState<Set<string>>(
    () => new Set((baseFixture?.swimlaneGroups ?? []).filter((g) => g.collapsed).map((g) => g.id)),
  );
  const data: RoadmapData = baseFixture
    ? { ...baseFixture, swimlaneGroups: (baseFixture.swimlaneGroups ?? []).map((g) => ({ ...g, collapsed: collapsedIds.has(g.id) })) }
    : sampleRoadmap;
    ...
    <div className="min-h-screen bg-zinc-50 p-8 dark:bg-black">
      <div className="relative mx-auto max-w-[1600px]">
        ...
        <RoadmapTimeline
          ...
          groupBandVariant={mergedMode ? "header" : groupVariant}
      ...
      {mergedMode && (
        ...
        >
          <b>wayframe#104</b> — All-Programs merged view (3 Programs, shared axis = union of date ranges)
        </div>
      )}
    </div>
  );
