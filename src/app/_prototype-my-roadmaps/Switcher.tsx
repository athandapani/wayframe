...
// PROTOTYPE — throwaway. Floating variant switcher for the 3 "My Roadmaps"
// landing-page layouts, per the /prototype skill's UI convention. wayframe#122.
import { useRouter, usePathname, useSearchParams } from "next/navigation";
import { useCallback, useEffect } from "react";

export const VARIANTS = [
  { key: "a", label: "A — Dense sorted table" },
  { key: "b", label: "B — Card grid, owned/shared split" },
  { key: "c", label: "C — Sidebar list + detail pane" },
] as const;

export type VariantKey = (typeof VARIANTS)[number]["key"];

export function useRoadmapsVariant(): VariantKey {
  const searchParams = useSearchParams();
  const v = searchParams.get("variant");
  return (VARIANTS.some((x) => x.key === v) ? v : "a") as VariantKey;
}

export function RoadmapsSwitcher() {
  const current = useRoadmapsVariant();
  ...
  return (
    <div className="fixed bottom-4 left-1/2 z-50 flex -translate-x-1/2 items-center gap-3 rounded-full border border-emerald-400 bg-emerald-950/90 px-4 py-2 text-sm text-white shadow-lg shadow-emerald-900/40">
      ...
      <span className="min-w-[280px] text-center font-mono">{label}</span>
