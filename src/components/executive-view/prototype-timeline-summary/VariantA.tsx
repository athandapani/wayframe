// PROTOTYPE (wayframe#37) — throwaway. Variant A: mini point-marker timeline
// strip, proportionally spaced between the first and last key date. Labels
// are short (same short-label/hover-for-full-title convention as
// RoadmapTimeline's markers) and greedily assigned to one of four tiers to
// avoid overlap when the critical path has many close-together milestones —
// a scaled-down version of the collision problem RoadmapTimeline's
// label-layout.ts already solves for the full chart. Solid diamonds are on
// the critical path (drive the finish date); hollow diamonds are flagged
// at-risk/delayed but off the critical chain — two different reasons a date
// matters, kept visually distinct rather than conflated.
...
export function VariantA({ summary }: { summary: ExecutiveTimelineSummary }) {
