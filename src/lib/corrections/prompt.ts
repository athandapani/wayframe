...
- date vs. potentialDate: use field=date only for a confirmed change to the milestone's committed date (e.g. "push X to March 15", "X actually landed on the 3rd"). Use field=potentialDate instead when the request describes a date the milestone merely *might* slip to — a risk, not a decision (e.g. "X could slip to March 15 if the vendor is late", "flag X as at risk of landing in April", "there's a chance Y won't ship until Q2"). potentialDate never moves the committed date and doesn't stamp originalDate; an empty string clears it (the risk didn't materialize, or was withdrawn).
...
- If the request asks to shift many milestones and/or PROGRAM-band items by the same amount at once (e.g. "push everything in the Manufacturing lane two weeks later", "pull the whole timeline forward from Field Pilot Kickoff by 10 days", "delay UL 3100 Certification, Fleet Coordination, and GA Milestone by a week"), use "bulkShiftOps": { selector, deltaDays, reason } instead of emitting one ops/topLevelItemOps date edit per item — do not compute each item's new date yourself. selector is exactly one of: { kind: "lane", laneId } (a real lane id, or the literal "PROGRAM" for the whole PROGRAM band); { kind: "after", afterId } (a real milestone or top-level-item id — every item dated on or after that item's own current date shifts, across lanes and the PROGRAM band together, including the referenced item itself); or { kind: "ids", ids: [...] } (specific real milestone/top-level-item ids named directly, mixed freely — use this for "shift A, B, and C" style requests). deltaDays is an integer, positive for later and negative for earlier — convert whatever unit the request uses (1 week=7, 1 month=30, 1 quarter=90, 1 year=365).
- Do not compute cascading effects on other, non-matched milestones — that is handled separately, deterministically, from the dependency graph. Only emit ops for milestones the request text is actually about. (This doesn't apply to bulkShiftOps' own selector, which deliberately covers many items at once.)
- If the request asks to accept a milestone's slip — clearing its drift baseline so it stops reading as ghosted/slipped (e.g. "accept the slip on UL 3100 Certification", "this is the new normal for Fleet Coordination", "clear the ghost on GA Milestone") — use "acceptBaselineOps": { scope: "one", targetId, reason } for one or more specifically-named milestones. If the request instead asks to accept every slip at once (e.g. "accept all the slips", "clear every ghost", "these are all the new normal now"), use a single { scope: "all", reason } entry instead — do not enumerate every ghosted milestone's targetId yourself. Never use field=date for this; a date op moves the committed date and stamps a *new* baseline, the opposite of what accepting a slip means.
