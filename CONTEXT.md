...
**Export Snapshot**:
An immutable, timestamped freeze of a Deck IR (#t28) rendering of the plan — the same 5-checkbox section selection (#t29: Executive / Combined Programs / Individual Programs / Scenario Combined / Scenario Program) an export uses, captured with a fixed canonical viewer-preference style profile so two people snapshotting the same document at the same moment get byte-identical output. Stored as fully-resolved shapes/text/connectors (never a raw CRDT state vector or a baked rendered file), so it stays readable as the document schema moves on, with no migration-on-read step. View-only forever; cannot be edited or re-baselined. Distinct from Baseline (the live, editable default Scenario) and from Scenario (a live, editable alternate plan) — an Export Snapshot freezes whichever of them the creator's selection included, not automatically every Program/Scenario at once (#t31 corrected this from this glossary's earlier "always everything" wording).
_Avoid_: Snapshot unqualified in UI copy (#128 — it collided with Version, below; the type names, routes and `portfolio_snapshots` table still say "snapshot", only what a user reads was renamed), Backup (an Export Snapshot is a deliberate archival record, not an autosave)

**Version**:
A manual, whole-Roadmap freeze of every Program's *document* — the plan itself, re-renderable as a roadmap — captured when someone clicks "Save a version" (#119: manual only; there is no automatic checkpoint anywhere). View-only: browsing a Version renders the ordinary combined canvas with every mutation callback withheld, and restoring one back onto the live document is deliberately not built (#128's own scope cut). Content is append-only and undeletable, inheriting #t11's ruling; the *label* is the one mutable field, because #127 made naming post-hoc (one click to save, then rename the row that appears).

Distinct from Export Snapshot above on the axis that matters: an Export Snapshot freezes a *rendering* (a Deck IR, for re-downloading a deck exactly as it looked), a Version freezes the *documents* (for reading the plan as it was). They also differ in scope — an Export Snapshot captures whichever of the 5 export sections its creator selected, a Version always covers the whole Roadmap — and in where the content comes from: a Deck IR can only be produced by measuring a rendered chart in a browser, so the client sends it, while a Version's Program documents are read server-side (`loadLiveProgramsForPortfolio`), which is why no client-sent document decides a Version's content.
_Avoid_: Snapshot (the collision this concept's naming exists to remove), Backup, Restore point (nothing restores yet)
